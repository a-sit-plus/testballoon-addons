...
internal class CompactRun(
    ...
    suspend fun failure(path: List<String>, error: Throwable) = mutex.withLock {
        failureCount += 1
        completed += 1
        if (config.report != CompactReport.SummaryOnly && withinLimit(failureDetails.size, config.reportRows)) {
            failureDetails += CompactFailure(path, error)
        } else {
            omittedFailures += 1
        }
    }
        ...
        val report = buildString {
            ...
            if (config.report != CompactReport.SummaryOnly) {
                ...
                failureDetails.take(failureRows).forEach { failure ->
                    ...
                    appendLine("  error: ${failure.error::class.simpleName}: ${failure.error.message}")
                    renderedRows += 1
                }
...
internal suspend fun runVirtualNodes(
    ...
    for (node in nodes) {
        when (node) {
            is VirtualNode.Suite -> if (!node.disabled) runVirtualNodes(node.children, testScope, run, path + node.name)
            is VirtualNode.DynamicSuite -> if (!node.disabled) runVirtualNodes(node.children(), testScope, run, path + node.name)
            is VirtualNode.Test -> {
            ...
            is VirtualNode.Data -> if (!node.disabled) runDataNode(node, testScope, run, path)
            is VirtualNode.DataTest -> if (!node.disabled) runDataTestNode(node, testScope, run, path)
            is VirtualNode.Property -> if (!node.disabled) runPropertyNode(node, testScope, run, path)
            is VirtualNode.PropertyTest -> if (!node.disabled) runPropertyTestNode(node, testScope, run, path)
        }
    }
}
...
private suspend fun runDataNode(
    ...
    suspend fun runOne(index: Long, value: Any?) {
        ...
        runVirtualNodes(node.body(value), testScope, run, path + name)
    }
